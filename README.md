...
Сервисы отдают виджеты либо html из которого собирается страница.

Еще один подход это написание api, которое будет уже общаться с внутренними сервисами. Но такое апи для каждого сайта должно быть своё.
...
```
### CMS
Поставить впереди свой собственный сервис, который отдает компонент (или имитирует cms), а потом интегрировать его с cms.

Со старыми системами хорошо работает фасад, чтобы закрывать апи, которое не подходит.

# Разбиение монолита на части
**Стык** - ограниченные контексты микросервисов.

Этапы разбиения
* Выделить разные функциональные части
* Создать пакеты, представляющие контексты и переместить в них код

При разбиении связи внешнего ключа в базе. Разносится на разные сервисы и взаимодействие идет через апи. 
